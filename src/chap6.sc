object chap6  {
  ...
  positiveMax(1000).run(RNG.simple(System.currentTimeMillis))
                                                  //> res0: (Int, chapter6.Randoms.RNG) = (926,chapter6.Randoms$RNG$$anon$1@1102b5
                                                  //| a9)
  /** Ex 1, 9 */
  ...
  positiveInt.run(RNG.simple(System.currentTimeMillis))
                                                  //> res1: (Int, chapter6.Randoms.RNG) = (1998790317,chapter6.Randoms$RNG$$anon$1
                                                  //| @45a51ac0)
  /** Ex 2, 6 */
  ...
  def intDouble: Rand[(Int, Double)] =
    map2(nextInt, nextDouble) { (i, d) => (i, d) }
                                                  //> intDouble: => chapter6.Randoms.Rand[(Int, Double)]
  def flip[A,B](in: (A, B)): (B, A) = in match { case (a, b) => (b, a) }
                                                  //> flip: [A, B](in: (A, B))(B, A)  def doubleInt: Rand[(Double, Int)] =
    intDouble map flip
                                                  //> doubleInt: => chapter6.Randoms.Rand[(Double, Int)]
  intDouble.run(RNG.simple(10000))
                                                  //> res2: ((Int, Double), chapter6.Randoms.RNG) = ((-447478295,0.64238026581815
                                                  //| 45),chapter6.Randoms$RNG$$anon$1@91619a4)  doubleInt.run(RNG.simple(20000))
                                                  //> res3: ((Double, Int), chapter6.Randoms.RNG) = ((0.7132686803644843,-8949565
  ...
  def double3: Rand[(Double, Double, Double)] =
    for {
    	...
    	d3 <- nextDouble
                                                  ...
                                                  //> double3: => chapter6.Randoms.Rand[(Double, Double, Double)]  
  double3.run(RNG.simple(9000))
                                                  //> res4: ((Double, Double, Double), chapter6.Randoms.RNG) = ((0.38753598760233
  ...
  ints(10).run(RNG.simple(10))
                                                  //> res6: (List[Int], chapter6.Randoms.RNG) = (List(3847489, 1334288366, 148686
