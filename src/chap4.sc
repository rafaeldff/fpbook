object chap4 {
  ...
  def parsePatterns(a:List[String]): Option[List[Pattern]] =
    sequence(a map pattern)                       //> parsePatterns: (a: List[String])Option[List[java.util.regex.Pattern]]
    
  parsePatterns(List("abcd", "^$"))               //> res19: Option[List[java.util.regex.Pattern]] = Some(List(abcd, ^$))
  parsePatterns(List())                           //> res20: Option[List[java.util.regex.Pattern]] = Some(List())
  parsePatterns(List("abcd", "["))                //> res21: Option[List[java.util.regex.Pattern]] = None
  ...
  }                                               //> traverse: [A, B](l: List[A])(f: A => Option[B])Option[List[B]]
  
  def sequence2[T](a: List[Option[T]]): Option[List[T]] =
    traverse[Option[T],T](a)(identity _)          //> sequence2: [T](a: List[Option[T]])Option[List[T]]

  sequence2(List(Option(1),Option(2)))            //> res22: Option[List[Int]] = Some(List(1, 2))
  sequence2(List(Option(1)))                      //> res23: Option[List[Int]] = Some(List(1))
  sequence2(List[Option[Int]]())                  //> res24: Option[List[Int]] = Some(List())
  sequence2(List(None:Option[Int]))               //> res25: Option[List[Int]] = None
  sequence2(List(Some(10), None, Some(20)))       //> res26: Option[List[Int]] = None
    
  
  def parsePatterns2(a:List[String]): Option[List[Pattern]] = traverse(a)(pattern)
                                                  //> parsePatterns2: (a: List[String])Option[List[java.util.regex.Pattern]]
  
  parsePatterns2(List("abcd", "^$"))              //> res27: Option[List[java.util.regex.Pattern]] = Some(List(abcd, ^$))
  parsePatterns2(List())                          //> res28: Option[List[java.util.regex.Pattern]] = Some(List())
  parsePatterns2(List("abcd", "["))               //> res29: Option[List[java.util.regex.Pattern]] = None
